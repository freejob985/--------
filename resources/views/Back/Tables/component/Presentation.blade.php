...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            ...
            @foreach ($component as $item_customers)
            <?php
      if($item_customers->id === 4){
          $url=route('Qualification');
      }else if($item_customers->id === 11){
        $url=route('skills');
    }else if($item_customers->id === 1){
        $url=route('contact');
    }else if($item_customers->id === 12){
        $url=route('MyEducation');
    }else{
        $url="";
      }
      ?>
            ...
            <tr class="hed">
                ...
                <td>{!! btn($item_customers->pag,18) !!}</td>
                <td><a href="{{  $url }}" class="btn btn-warning">Link</a>
                </td>
            </tr>
            @endforeach
            @endif
