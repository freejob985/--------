...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            ...
            <tr class="hed">
                <td><span class="badge">{{ $item_gallery->id}}</span></td>
                <td><img class="materialboxed" width="{{ $Settings[4] }}"
                        src="{{Request::root()}}{!! $Settings[3] !!}{{ $item_gallery->Image}}">
                <td>{{ $item_gallery->Title}}</td>
                <td>{!! set_lab_lang($item_gallery->Language) !!}</td>
                <td>
                    <a rel="tooltip" href="{{ route('gallery.edit', ['id'=>$item_gallery->id]) }}"
                        class="btn btn-success btn-round btn-just-icon btn-sm" data-original-title="" title="">
                        <i class="material-icons">edit</i>
                    </a>
                    <button type="button" idt="{{ $item_gallery->id}}" idm="{{ $item_gallery->Image}}"
                        route="{{ route('gallery.destroy.dell', ['id'=>$item_gallery->id,'img'=>$item_gallery->Image]) }}"
    ...
    <br>
    <a style="background: #011a25;" href="{{ route('gallery.create') }}" class="btn btn-primary btn-block btn-large">
