...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            ...
            <tr class="hed">
                <td><span class="badge">{{ $item_slide->id}}</span></td>
                <td><img class="materialboxed" width="{{ $Settings[4] }}"
                        src="{{Request::root()}}{!! $Settings[3] !!}{{ $item_slide->img}}">
                <td>{{ $item_slide->address}}</td>
                <td>{{ $item_slide->Sub}}</td>
                <td>{!! cheek_vidoe_online($item_slide->YouTube) !!}</td>
                <td>{!! set_lab_lang($item_slide->language) !!}</td>
                <td>
                    <a rel="tooltip" href="{{ route('slide.edit', ['id'=>$item_slide->id]) }}"
                        class="btn btn-success btn-round btn-just-icon btn-sm" data-original-title="" title="">
                        <i class="material-icons">edit</i>
                    </a>
                    <button type="button" idt="{{ $item_slide->id}}" idm="{{ $item_slide->img}}"
                        route="{{ route('slide.destroy.dell', ['id'=>$item_slide->id,'img'=>$item_slide->img]) }}"
    ...
    <br>
    <a style="background: #011a25;" href="{{ route('slide.create') }}" class="btn btn-primary btn-block btn-large">
