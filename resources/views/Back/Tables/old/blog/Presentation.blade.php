...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            ...
            <tr class="hed">
                <td><span class="badge">{{ $item_blog->id}}</span></td>
                <td><img class="materialboxed" width="{{ $Settings[4] }}"
                        src="{{Request::root()}}{!! $Settings[3] !!}{{ $item_blog->Image}}">
                <td>{{ $item_blog->Title}}</td>
                <td>{{ $item_blog->Explanation}}</td>
                <td>{{ $item_blog->Date}}</td>
                <td>{!! set_lab_lang($item_blog->Language) !!}</td>
                <td>
                    <a rel="tooltip" href="{{ route('blog.edit', ['id'=>$item_blog->id]) }}"
                        class="btn btn-success btn-round btn-just-icon btn-sm" data-original-title="" title="">
                        <i class="material-icons">edit</i>
                    </a>
                    <button type="button" idt="{{ $item_blog->id}}" idm="{{ $item_blog->Image}}"
                        route="{{ route('blog.destroy.dell', ['id'=>$item_blog->id,'img'=>$item_blog->Image]) }}"
    ...
    <br>
    <a style="background: #011a25;" href="{{ route('blog.create') }}" class="btn btn-primary btn-block btn-large">
