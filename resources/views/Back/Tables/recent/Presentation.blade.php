...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            @if (count($recent)>0)
            @foreach ($recent as $item_section_work)
            <tr class="hed">
                ...
                <td>{{ $item_section_work->Explanation}}</td>
                <td>{!! set_lab_lang($item_section_work->Language) !!}</td>
                <td>
                    <a rel="tooltip" href="/admin/recent/{{ $item_section_work->id}}"
                    ...
                    <button type="button" idt="{{ $item_section_work->id}}" idm="{{ $item_section_work->Image}}"
                        route="{{Request::root()}}/admin/destroy/recent/{{ $item_section_work->id}}/{{ $item_section_work->Image}}"
