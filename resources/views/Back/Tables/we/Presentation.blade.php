...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            @if (count($we)>0)
            @foreach ($we as $item_section_work)
            <tr class="hed">
                <td><span class="badge">{{ $item_section_work->id}}</span></td>
                <td><img class="materialboxed" width="{{ $Settings[4] }}"
                    src="{{Request::root()}}{!! $Settings[3] !!}{{ $item_section_work->Component1}}"></td>
                <td>{{ $item_section_work->Component2}}</td>
                <td>{{ $item_section_work->Component3}}</td>
                <td>{!! set_lab_lang($item_section_work->Language) !!}</td>
                <td>
                    <a rel="tooltip" href="/admin/we/{{ $item_section_work->id}}"
                        class="btn btn-success btn-round btn-just-icon btn-sm" data-original-title="" title="">
                        <i class="material-icons">edit</i>
                    </a>

                    <button type="button" idt="{{ $item_section_work->id}}" idm="{{ $item_section_work->Component1}}"
                        route="{{Request::root()}}/admin/destroy/we/{{ $item_section_work->id}}/{{ $item_section_work->Component1}}"
