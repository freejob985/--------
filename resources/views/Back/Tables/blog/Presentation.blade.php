...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            @if (count($blog)>0)
            @foreach ($blog as $item_section_work)
            <tr class="hed">
                ...
                <td>{{ $item_section_work->Author}}</td>
                <td>{!! set_lab_lang($item_section_work->Language) !!}</td>
                <td>
                    <a rel="tooltip" href="/admin/blog/{{ $item_section_work->id}}"
                    ...
                    <button type="button" idt="{{ $item_section_work->id}}" idm="{{ $item_section_work->Image}}"
                        route="{{Request::root()}}/admin/destroy/blog/{{ $item_section_work->id}}/{{ $item_section_work->Image}}"
