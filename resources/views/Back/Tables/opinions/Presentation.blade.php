...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            @if (count($opinions)>0)
            @foreach ($opinions as $item_section_work)
            <tr class="hed">
                <td><span class="badge">{{ $item_section_work->id}}</span></td>
              <td><img class="materialboxed" width="{{ $Settings[4] }}"
                    src="{{Request::root()}}{!! $Settings[3] !!}{{ $item_section_work->image}}">
                <td>{{ $item_section_work->comment}}</td>
                <td>{{ $item_section_work->Name}}</td>
                <td>{{ $item_section_work->Job}}</td>
                <td>{!! set_lab_lang($item_section_work->Language) !!}</td>
                <td>
                    <a rel="tooltip" href="/admin/opinions/{{ $item_section_work->id}}"
                    ...
                    <button type="button" idt="{{ $item_section_work->id}}" idm="{{ $item_section_work->image}}"
                        route="{{Request::root()}}/admin/destroy/opinions/{{ $item_section_work->id}}/{{ $item_section_work->image}}"
