...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            @if (count($slide)>0)
            @foreach ($slide as $item_section_work)
            <tr class="hed">
                <td><span class="badge">{{ $item_section_work->id}}</span></td>
                <td>{{ $item_section_work->Subject}}</td>
                <td>{{ $item_section_work->Title}}</td>
                <td>{{ $item_section_work->Sub}}</td>
                <td>{{ $item_section_work->Additional}}</td>
                <td>{{ $item_section_work->Link}}</td>


                <td><img class="materialboxed" width="{{ $Settings[4] }}"
                    src="{{Request::root()}}{!! $Settings[3] !!}{{ $item_section_work->Image}}"></td>


                <td>{!! set_lab_lang($item_section_work->Language) !!}</td>

                <td>
                    <a rel="tooltip" href="/admin/slide/{{ $item_section_work->id}}"
                    ...
                    <button type="button" idt="{{ $item_section_work->id}}" idm="{{ $item_section_work->Image}}"
                        route="{{Request::root()}}/admin/destroy/slide/{{ $item_section_work->id}}/{{ $item_section_work->Image}}"
