...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            @if (count($skills)>0)
            @foreach ($skills as $item_section_work)
            <tr class="hed">
                <td><span class="badge">{{ $item_section_work->id}}</span></td>
                <td>{{ $item_section_work->Skill}}</td>
                <td>{{ $item_section_work->Rating}}</td>
                <td>{!! set_lab_lang($item_section_work->Language) !!}</td>
                <td>
                    <a rel="tooltip" href="/admin/skills/{{ $item_section_work->id}}"
                    ...
                    <button type="button" idt="{{ $item_section_work->id}}" idm="{{ $item_section_work->Image}}"
                        route="{{Request::root()}}/admin/destroy/skills/{{ $item_section_work->id}}"
